...
%             [Dx2, Dz2, Dx1z1, Dx3z1, Dx1z3, Dxz2, Dx4, Dz4] = FD_stencils(ix,iz,Pp,N,dx,dz);

            % Version 1  (VTI qP)
%             Qpp(iz,ix) = 2*Qp(iz,ix)-Qpm(iz,ix)+dt^2*vp0(iz,ix)^2*(Dx4+(1+2*epsilon(iz,ix))*Dz4+2*(1+delta(iz,ix))*Dxz2);
...
%     Qpm = Qp;     % Value exchange
%     Qp = Qpp;

    for iz = 1:Nz
        for ix = 1:Nx
            Qpm(iz,ix) = Qp(iz,ix)*ABCs(iz,ix);		   % 时间层赋值，附加边界条件%
            Qp(iz,ix) = Qpp(iz,ix)*ABCs(iz,ix);         % 时间层赋值，附加边界条件%
        end
    end
